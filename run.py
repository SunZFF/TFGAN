...
from metrics.freq_analyze import visualization_psd



def train(opt, ori_data, r_data):

    # Model Setting
    model = timegan.TimeGAN(opt, ori_data, r_data)
...
def test(opt, ori_data, r_data):
    ...
    real_size = opt.batch_size
    num = synth_size // real_size
    gen_data = list()
    for i in range(num):
        generated_data = model.gen_synth_data(real_size)
        for j in range(real_size):
            temp = generated_data[j, :opt.seq_len, :]
    ...
    visualization_psd(ori_data, gen_data, opt.output_dir)
