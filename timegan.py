...
class TimeGAN:
    def __init__(self, opt, ori_data, r_data):

        self.opt = opt
        self.ori_data, self.min_val, self.max_val = MinMaxScaler(ori_data)#用于反归一化时恢复原始数据
        self.r_data ,_ ,_ = MinMaxScaler(r_data)
        ...
        self.r_data = torch.tensor(self.r_data, dtype=torch.float32).to(self.device)
        ...
        self.para['resampled_fft_weight'] = self.opt.resampled_fft_weight
        #self.wight = self.opt.H_hat_weight
        self.embedder = Embedder(self.para).to(self.device)#实例化模型对象
        ...
        self.optim_embedder = torch.optim.Adam(self.embedder.parameters(), lr=self.opt.lr)#lr:学习率
        ...
        # Set loss function
        self.MSELoss = torch.nn.MSELoss()#均方误差损失
        self.BCELoss = torch.nn.BCELoss()#交叉熵损失

        if self.opt.load_checkpoint:#是否加载预训练模型
            self.load_trained_networks()

    def gen_batch(self):#生成训练所需的批量数据
    ...
    def batch_forward(self):
        ...
        # 保存 self.H 的数据
        torch.save(self.H.cpu().detach(), './trained_networks/H_data.pt')
        ...
        self.Y_real = self.discriminator(self.H, self.r_data)
        self.Y_fake = self.discriminator(self.H_hat, self.r_data)
        self.Y_fake_e = self.discriminator(self.E_hat, self.r_data)

    def gen_synth_data(self, batch_size):#用于生成合成数据
        self.H = torch.load('./trained_networks/H_data.pt').to(self.device)#!!!
        ...
        self.E_hat = self.generator(self.Z)
        self.H_hat = self.supervisor(self.E_hat)
        #self.H_hat = self.H_hat * self.wight +self.H * (1-self.wight)#####
        self.X_hat = self.recovery(self.H_hat)
        self.X_hat = self.X_hat .cpu().detach().numpy()
        self.X_hat = self.X_hat * self.max_val
        self.X_hat = self.X_hat + self.min_val
    ...
    def train_generator(self,join_train=False):
        ...
        self.optim_generator.zero_grad()
        self.optim_supervisor.zero_grad()
        self.G_loss_U = self.BCELoss(self.Y_fake, torch.zeros_like(self.Y_fake))
        self.G_loss_U_e = self.BCELoss(self.Y_fake_e, torch.zeros_like(self.Y_fake_e))
    ...
    def train_discriminator(self):
        ...
        self.D_loss_real = self.BCELoss(self.Y_real, torch.zeros_like(self.Y_real))
        self.D_loss_fake = self.BCELoss(self.Y_fake, torch.ones_like(self.Y_fake))
        self.D_loss_fake_e = self.BCELoss(self.Y_fake_e, torch.ones_like(self.Y_fake_e))
